...
async function answerConversationally(command, plan = {}, ceoIntent = {}) {
  return ExecutiveConversationService.respond({
    command,
    plan,
    ceoIntent,
...
async function handleCommand(command) {
  ...
  /*
    ...
    CEO intent is resolved before the operational planner. Questions,
    strategy, and decision support stay out of the execution queue.
  */
  const ceoIntent = CEOIntentEngineService.analyze({
    command: cleanCommand
  });

  if (!ceoIntent.shouldQueue) {
    const conversation = await answerConversationally(cleanCommand, {}, ceoIntent);

    log('INFO', 'CEO intent answered without queueing', {
      ceoIntent: ceoIntent.intent,
      route: ceoIntent.route,
      confidence: ceoIntent.confidence
    });
    ...
    return {
      ...
      status: 'ANSWERED',
      mode: 'CONVERSATION',
      interactionType: ceoIntent.intent,
  ...
  const plan = CommandIntentPlannerService.plan({
    command: cleanCommand,
    intent: ceoIntent.intent
  });

  const operation = makeOperation(cleanCommand, plan);
  operation.ceoIntent = ceoIntent;
