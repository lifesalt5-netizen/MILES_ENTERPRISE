# INSTALL_MILES_BUILD_019_REVENUE_FIRST_AUTONOMOUS_COO.ps1
...
#
# Fixes:
#   - Await asynchronous ExecutiveIntelligenceService.getExecutiveState()
#   - Generate revenue-first business objectives
#   - Route operational objectives, not generic mission titles
#   - Preserve existing Planner -> Workflow -> Execution -> Verification path
...
$BackupRoot = Join-Path $Root "_BACKUPS\BUILD_019_$Stamp"
$ReportDir = Join-Path $Root "DATA\build_019"
...
if (-not (Test-Path $TargetPath)) {
    throw "Authoritative AutonomousCOOLoopService not found: $TargetPath"
}
...
Write-Host ""
Write-Host "============================================================"
Write-Host "MILES BUILD 019"
Write-Host "Revenue-First Autonomous COO"
...
$TestPath = Join-Path $TestDir "Test_Build019_RevenueFirstAutonomousCOO.js"
...
$SyntaxFiles = @(
    ...
    ".\TESTS\Test_Build019_RevenueFirstAutonomousCOO.js"
)
...
Write-Host ""
Write-Host "=== BUILD 019 AUTOMATED TESTS ==="

$TestOutput = & node $TestPath 2>&1
$TestExit = $LASTEXITCODE
$TestReport = Join-Path $ReportDir "build_019_test_$Stamp.txt"

$TestOutput | Tee-Object -FilePath $TestReport

if ($TestExit -ne 0) {
    throw "Build 019 automated tests failed. Restore from: $BackupRoot"
}

Write-Host ""
Write-Host "=== ONE LIVE COO CYCLE ==="

$LiveOutput = & node ".\StartAutonomousCOO.js" 2>&1
$LiveExit = $LASTEXITCODE
$LiveReport = Join-Path $ReportDir "build_019_live_cycle_$Stamp.txt"

$LiveOutput | Tee-Object -FilePath $LiveReport

if ($LiveExit -ne 0) {
    throw "Build 019 live COO cycle failed. Restore from: $BackupRoot"
}

$Manifest = [ordered]@{
    ok = $true
    build = "019"
    name = "Revenue-First Autonomous COO"
    ...
    fixes = @(
        "Await ExecutiveIntelligenceService.getExecutiveState",
        "Use normalized business state for replies, proposals, deals, campaigns, contractors, and opportunities",
        "Generate revenue-first objectives",
        "Send operational objectives into WorkflowService instead of generic mission titles"
    )
    testReport = $TestReport
    liveCycleReport = $LiveReport
}

$ManifestPath = Join-Path $ReportDir "build_019_manifest_$Stamp.json"
...
Write-Host ""
Write-Host "============================================================"
Write-Host "BUILD 019 INSTALLED AND VERIFIED"
Write-Host "============================================================"
Write-Host "Backups: $BackupRoot"
Write-Host "Reports: $ReportDir"
Write-Host ""
Write-Host "Production start command:"
