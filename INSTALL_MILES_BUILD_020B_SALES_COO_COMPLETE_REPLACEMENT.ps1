# INSTALL_MILES_BUILD_020B_SALES_COO_COMPLETE_REPLACEMENT.ps1
# Complete replacement installer based on authoritative uploaded files.
...
$BackupRoot =
    Join-Path $Root "_BACKUPS\BUILD_020B_$Stamp"

$ReportDir =
    Join-Path $Root "DATA\build_020"
...
'@ | Set-Content `
    -Path ".\PROVIDERS\providers\SalesProvider.js" `
    -Encoding UTF8
...
'@ | Set-Content `
    -Path ".\TESTS\Test_Build020_SalesCOO.js" `
    -Encoding UTF8

Write-Host ""
Write-Host "=== BUILD 020B SYNTAX VALIDATION ==="
...
Write-Host ""
Write-Host "=== BUILD 020B AUTOMATED TESTS ==="

$Output =
    & node ".\TESTS\Test_Build020_SalesCOO.js" 2>&1
...
$Report =
    Join-Path `
        $ReportDir `
        "build_020B_test_$Stamp.txt"

$Output |
    Tee-Object -FilePath $Report

if ($ExitCode -ne 0) {
    throw "Build 020B tests failed. Restore from $BackupRoot"
}

$Manifest = [ordered]@{
    ok = $true
    build = "020B"
    name = "Sales COO Complete Replacement"
    installedAt =
        (Get-Date).ToString("o")
    ...
    safety = @(
        ...
        "Positive replies and submission actions are marked for protected review"
    )
    report = $Report
}
...
$Manifest |
    ...
    Set-Content `
        -Path (
            Join-Path `
                $ReportDir `
                "build_020B_manifest_$Stamp.json"
        ) `
        -Encoding UTF8

Write-Host ""
Write-Host "============================================================"
Write-Host "BUILD 020B SALES COO INSTALLED AND VERIFIED"
Write-Host "============================================================"
