...
class CommandIntentPlannerService {
  ...
  isRevenueOperationsMission(text) {
    ...
    /expand outbound/.test(text) ||
    /increase booked meetings/.test(text) ||
    /create.*mailbox/.test(text) ||
    /create.*email.*domain/.test(text) ||
    /assign.*campaign/.test(text) ||
    /upload.*lead/.test(text) ||
    /match.*lead.*segment/.test(text) ||
    /track.*response/.test(text) ||
    /run.*campaign/.test(text)
);
  }
