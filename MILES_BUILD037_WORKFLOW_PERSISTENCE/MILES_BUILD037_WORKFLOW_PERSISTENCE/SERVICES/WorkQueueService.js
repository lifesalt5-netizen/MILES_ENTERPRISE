...
class WorkQueueService {
    constructor(options = {}) {
        ...
        this.lockTimeoutMs = Number(
            options.lockTimeoutMs ||
            process.env.MILES_WORK_QUEUE_LOCK_TIMEOUT_MS ||
            15000
        );

        this.staleLockMs = Number(
            options.staleLockMs ||
            process.env.MILES_WORK_QUEUE_STALE_LOCK_MS ||
            30000
        );
    ...
    withLock(fn) {
        ...
        if (!this.acquireLock()) {
            throw new Error(
                `WorkQueueService lock could not be acquired: ${this.lockPath}`
            );
        }
    ...
    save() {
        return this.withLock(() => this.saveUnlocked());
    }
    ...
    createWorkItem(input = {}) {
        return this.withLock(() => {
            ...
            if (existing) {
                ...
                existing.requiresKevin =
                    this.normalizeBoolean(existing.requiresKevin);
                ...
                const workflowResult =
                    existing.metadata?.workflowResult || {};
                ...
                const workPackageId =
                    ...
                    existing.metadata?.workPackageId ||
                    null;

                const orphanQueued =
                    existing.executionType === "WORKFLOW" &&
                    ["Queued", "QUEUED"].includes(existing.status) &&
                    !workPackageId &&
                    queuedTasks === 0 &&
                    workPackageTasks === 0;
                ...
                if (orphanQueued) {
                    ...
                    this.addLifecycle(
                        existing,
                        "Pending",
                        "Recovered queued workflow with no persisted work package or executable task evidence."
                    );

                    console.log(
                        "[F001] Recovered orphan workflow:",
                        existing.id
                    );
                } else {
                    this.addLifecycle(
