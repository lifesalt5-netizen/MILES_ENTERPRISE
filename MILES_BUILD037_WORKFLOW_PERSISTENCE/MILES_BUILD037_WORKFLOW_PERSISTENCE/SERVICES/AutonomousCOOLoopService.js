...
class AutonomousCOOLoopService {
  ...
  queueAuthorizedWorkflows() {
    ...
    if (
      this.workQueue &&
      typeof this.workQueue.load === "function"
    ) {
      this.workQueue.load();
      console.log(
        "[D015] WorkQueue reloaded before authorization check."
      );
    }
    ...
    const authorized =
      this.workQueue &&
      typeof this.workQueue.getAuthorizedPending === "function"
    ...
    console.log(
      `[D015] Total Work Items: ${(allItems || []).length}`
    );
    console.log(
      `[D015] Authorized Pending: ${authorized.length}`
    );
    ...
    for (const item of authorized) {
      ...
      try {
        ...
        if (existingWorkPackageId) {
          ...
          const persisted = this.persistQueuedState(item.id, {
        ...
        const taskEvidence =
          this.hasWorkflowTaskEvidence(workPackageId);

        if (
          (!persisted || persisted.status !== "Queued") &&
          !taskEvidence
        ) {
          throw new Error(
        ...
        results.push({
          ...
          persistedStatus:
            persisted?.status ||
            (taskEvidence ? "Queued" : null),
          persistenceRecoveredFromTaskEvidence:
            Boolean(
              taskEvidence &&
              (!persisted || persisted.status !== "Queued")
            )
        ...
        console.error(
          `[D015] FAILED Work Item ${item?.id || "UNKNOWN"}`
        );
        console.error(err);

        const workflowWasCreated =
          Boolean(workPackageId) ||
          queuedTasks > 0 ||
          this.hasWorkflowTaskEvidence(workPackageId);

        if (workflowWasCreated) {
          const recovered = this.persistQueuedState(item.id, {
            queuedBy: "AutonomousCOOLoopService",
            queuedAt: now(),
            workflowStatus: workflowResult?.status || "QUEUED",
            workPackageId,
            workflowResult,
            sourceWorkItemId: item.id,
            persistenceRecoveredAfterError: true,
          ...
          results.push({
            ...
            queuedTasks,
            workPackageId,
            persistedStatus: recovered?.status || "Queued",
            warning: err.message
          });

          console.warn(
            `[D015] RECOVERED ${item.id}; workflow/task evidence exists, so the work item was not marked Failed.`
          );
  ...
  hasWorkflowTaskEvidence(workPackageId) {
    if (
      !workPackageId ||
      !taskQueue ||
    ...
    try {
      return taskQueue.list().some(
        task =>
          task &&
          task.payload?.workPackageId === workPackageId
      );
    } catch {
      return false;
    }
  ...
  persistQueuedState(id, metadata = {}) {
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
      try {
        ...
        if (
          typeof this.workQueue.markQueued === "function"
        ) {
          lastUpdated =
            this.workQueue.markQueued(id, {
              ...metadata,
        ...
        const verified =
          typeof this.workQueue.getAll === "function"
            ? (this.workQueue.getAll() || []).find(
                row => row && row.id === id
              )
            : lastUpdated;

        if (verified?.status === "Queued") {
          return verified;
        }
      } catch (error) {
        console.warn(
          `[D015] Work item ${id} queue persistence attempt ${attempt} failed: ${error.message}`
        );
      }
    ...
    const workPackageId =
      metadata.workPackageId ||
      metadata.workflowResult?.workPackage?.id ||
      metadata.workflowResult?.workPackageId ||
      null;

    if (this.hasWorkflowTaskEvidence(workPackageId)) {
      return {
        ...(lastUpdated || {}),
        id,
        status: "Queued",
        metadata: {
          ...
          ...metadata,
          persistenceVerifiedByTaskEvidence: true,
          persistenceVerifiedAt: now()
        }
      };
    }
