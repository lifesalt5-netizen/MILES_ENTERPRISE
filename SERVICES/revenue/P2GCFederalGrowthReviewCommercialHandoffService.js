...
class P2GCFederalGrowthReviewCommercialHandoffService{
  ...
  buildDraft(reviewId,input={}){
    ...
    const draft={version:1,reviewId,createdAt:isoNow(),company:record.company?.name||null,recipient:record.recipient?.email||null,packageName:clean(input.packageName)||null,scopeSummary:clean(input.scopeSummary)||null,price:money(input.price),currency:clean(input.currency||'USD').toUpperCase(),proposalRef:clean(input.proposalRef)||null,paymentUrl:httpsUrl(input.paymentUrl),approvedByKevin:input.approvedByKevin===true,approvalNote:clean(input.approvalNote)||null,proposalStatus:clean(input.proposalStatus||'DRAFT').toUpperCase(),paymentStatus:'NOT_PAID',guards:{pricingInvented:false,automaticCharge:false,finalOfferRequiresKevin:true,httpsPaymentRequired:true}};
    ...
    this.lifecycle.completeStage(reviewId,'PACKAGE_PROPOSAL_PAYMENT_HANDOFF',{source:'P2GC_COMMERCIAL_HANDOFF',freshness:draft.createdAt,confidence:'HIGH',verificationState:'CONFIRMED',artifact:file,notes:`Kevin-approved package ${draft.packageName}; proposal ${draft.proposalRef}; payment handoff configured`});
