...
class AwardHistoryPersistenceService {
  constructor(options = {}) {
    this.dbPath = options.dbPath || process.env.ORION_DB || process.env.ORION_DB_PATH ||
  ...
  loadDatabase() {
    ...
    // Lazy load keeps plan/validation paths usable without opening ORION.
  ...
  ensureSchema(db) {
    db.exec(`
      ...
      CREATE TABLE IF NOT EXISTS award_history_refresh_runs (
        ...
        federal_revenue REAL NOT NULL DEFAULT 0,
        award_count INTEGER NOT NULL DEFAULT 0,
        prime_awarded_revenue REAL NOT NULL DEFAULT 0,
        prime_award_count INTEGER NOT NULL DEFAULT 0,
        subcontracted_revenue REAL NOT NULL DEFAULT 0,
        subcontract_award_count INTEGER NOT NULL DEFAULT 0,
  ...
  resolveContractor(db, uei) {
    ...
    if (rows.length !== 1) {
      throw new Error(rows.length === 0 ? `ORION contractor not found for UEI ${uei}` : `ORION UEI is not unique: ${uei}`);
    }
    return rows[0];
  }
  ...
  persist(audit = {}, options = {}) {
    ...
    try {
      ...
      const insertPrime = db.prepare(`
        INSERT INTO award_history_prime (
          contractor_id, uei, award_id, recipient_name, start_date, end_date, amount,
          description, awarding_agency, awarding_sub_agency, funding_agency, funding_sub_agency,
          award_type, source, refreshed_at
        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `);

      const insertSub = db.prepare(`
        INSERT INTO award_history_subcontracts (
          contractor_id, uei, subaward_id, prime_award_id, recipient_name, recipient_uei,
          action_date, amount, description, awarding_agency, source, refreshed_at
        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `);
      ...
      const transaction = db.transaction(() => {
        ...
        for (const row of (audit.primeAwards || [])) {
          insertPrime.run(
            contractor.id, validation.uei, clean(row.awardId), row.recipientName || null,
            row.startDate || null, row.endDate || null, number(row.amount), row.description || null,
            row.awardingAgency || null, row.awardingSubAgency || null, row.fundingAgency || null,
            row.fundingSubAgency || null, row.awardType || null, row.source || "USAspending.gov", persistedAt
          );
        }

        for (const row of (audit.subcontracts || [])) {
          const stableSubawardId = clean(row.subawardId) || sha256([
            row.primeAwardId, row.recipientName, row.actionDate, number(row.amount)
          ].map(clean).join("|"));
          insertSub.run(
            contractor.id, validation.uei, stableSubawardId, row.primeAwardId || null,
            row.recipientName || null, row.recipientUei || null, row.actionDate || null,
            number(row.amount), row.description || null, row.awardingAgency || null,
            row.source || "USAspending.gov", persistedAt
          );
        }

        db.prepare("UPDATE contractors SET federal_revenue = ?, award_count = ? WHERE id = ?").run(
          validation.summary.federalRevenue,
          validation.summary.awardCount,
          contractor.id
        );

        db.prepare(`
          INSERT INTO award_history_refresh_runs (
            contractor_id, uei, generated_at, persisted_at, identity_authority, identity_method,
            federal_revenue, award_count, prime_awarded_revenue, prime_award_count,
            subcontracted_revenue, subcontract_award_count, audit_fingerprint
          ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        `).run(
          contractor.id,
          validation.uei,
          audit.generatedAt || persistedAt,
          persistedAt,
          audit?.source?.identityAuthority || null,
          audit?.source?.recipientMatchedBy || null,
          validation.summary.federalRevenue,
          validation.summary.awardCount,
          validation.summary.primeAwardedRevenue,
          validation.summary.primeAwardCount,
          validation.summary.subcontractedRevenue,
          validation.summary.subcontractAwardCount,
          auditFingerprint
        );
      });
      ...
      const verified =
        number(after.federal_revenue) === validation.summary.federalRevenue &&
        number(after.award_count) === validation.summary.awardCount &&
        number(primeRows.count) === validation.summary.primeAwardCount &&
        number(subRows.count) === validation.summary.subcontractAwardCount &&
        Math.abs(number(primeRows.revenue) - validation.summary.primeAwardedRevenue) <= 0.005 &&
        Math.abs(number(subRows.revenue) - validation.summary.subcontractedRevenue) <= 0.005;
      ...
      return {
        ...
        summary: validation.summary,
        ledger: {
          primeRows: number(primeRows.count),
          primeRevenue: number(primeRows.revenue),
          subcontractRows: number(subRows.count),
          subcontractRevenue: number(subRows.revenue)
        },
        backupPath,
        auditFingerprint,
