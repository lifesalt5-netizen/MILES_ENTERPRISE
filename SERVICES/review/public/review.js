...
function render(review,watermark){
  current=review;$('company').textContent=review.company?.name||'Federal Growth Review';$('expires').textContent=`Secure access expires ${new Date(review.expiresAt).toLocaleString()}`;$('fit').textContent=Math.round(review.scoring?.fitScore||0);$('intent').textContent=Math.round(review.scoring?.intentScore||0);$('priority').textContent=Math.round(review.scoring?.salesPriority||0);$('watermark').textContent=watermark?.label||'';$('runtime').textContent=review.runtime?.display||review.runtime?.formatted||'Target presentation length: approximately 6–10 minutes.';
  ...
  if(review.video?.status==='READY'&&review.video.mediaId) setupVideo(review.video.mediaId);else $('videoPending').hidden=false;
}
async function setupVideo(mediaId){
  try{const result=await api('video-token',{method:'POST',body:JSON.stringify({mediaId})});const v=$('video');v.hidden=false;$('videoPending').hidden=true;v.dataset.mediaId=mediaId;v.dataset.token=result.token;attachVideoTelemetry(v);}catch{ $('videoPending').hidden=false; }
}
