...
const rootDir = path.resolve(process.env.P2GC_GROWTH_CHILD_ROOT || process.env.MILES_ROOT || path.resolve(__dirname, '..', '..'));
const term = String(process.env.P2GC_GROWTH_CHILD_TERM || '').trim();
const refresh = String(process.env.P2GC_GROWTH_CHILD_REFRESH || '').trim() === '1';
...
buildModel()
  .then(model => {
    if (typeof process.send === 'function') process.send({ ok:true, model });
    else process.stdout.write(JSON.stringify({ ok:true, model }));
  })
  .catch(error => {
    const payload = { ok:false, error:String(error?.stack || error?.message || error) };
