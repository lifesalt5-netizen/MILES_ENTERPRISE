...
function identity(m){const u=ueiFrom(m);return u?'UEI:'+u:(nameFrom(m)?'NAME:'+nameFrom(m):null)}
...
async function loadVehicle(rec,file){if(!file)return{file:null,rows:0};let rows=0;const meta=await streamCsv(file,row=>{const m=fm(row),id=identity(m);if(!id)return;const vehicle=s(first(m,['vehicle','fed_vehicle','contract_vehicle'])).toUpperCase();const r=ensure(rec,id);const nm=nameFrom(m);if(nm)r.names.add(nm);r.sources.add(file);if(/\bGSA\b|\bMAS\b|MULTIPLE AWARD SCHEDULE/.test(vehicle)){r.families.add('GSA');r.evidence.push('VEHICLE:GSA')}if(/\bVA\b|\bFSS\b|VETERANS AFFAIRS|FEDERAL SUPPLY SCHEDULE/.test(vehicle)){r.families.add('VA_FSS');r.evidence.push('VEHICLE:VA_FSS')}const rv=money(first(m,['federal_total_revenue_x','federal_total_revenue','total_revenue','federal_revenue']));if(rv!=null)r.revenues.push(rv);rows++});return{file,rows,headers:meta.headers}}
async function loadRevenue(rec,file){if(!file)return{file:null,rows:0};let rows=0;const meta=await streamCsv(file,row=>{const m=fm(row),id=identity(m);if(!id)return;const r=ensure(rec,id);const nm=nameFrom(m);if(nm)r.names.add(nm);r.sources.add(file);const rv=money(first(m,['Federal_Total_Revenue','NORMALIZED_REVENUE','federal_total_revenue','total_revenue','federal_revenue']));if(rv!=null)r.revenues.push(rv);rows++});return{file,rows,headers:meta.headers}}
async function loadCert(rec,file){if(!file)return{file:null,rows:0};let rows=0;const meta=await streamCsv(file,row=>{const m=fm(row),id=identity(m);if(!id)return;const r=ensure(rec,id);const nm=nameFrom(m);if(nm)r.names.add(nm);r.sources.add(file);const blob=[first(m,['Active SBA certifications']),first(m,['Business type and self-certifications'])].map(s).join(' ').toUpperCase();const tests=[['SDVOSB',/SDVOSB|SERVICE[- ]DISABLED VETERAN/,['is_sdvosb']],['VOSB',/(^|\W)VOSB(\W|$)|VETERAN[- ]OWNED/,['is_vosb','Veteran-Owned Small Business (VOSB) certification status']],['WOSB',/WOSB|WOMEN[- ]OWNED/,['is_wosb','Women-Owned Small Business (WOSB) certification status']],['HUBZONE',/HUB ?ZONE/,['is_hubzone','HUBZone certification status']],['8A',/8\(A\)|(^|\W)8A(\W|$)/,['is_8a','8(a) certification status']]];for(const [fam,re,fields] of tests){if(re.test(blob)||fields.some(f=>truthy(first(m,[f])))){r.families.add(fam);r.evidence.push('CERT:'+fam)}}const rv=money(first(m,['federal_revenue','Federal_Total_Revenue','NORMALIZED_REVENUE']));if(rv!=null)r.revenues.push(rv);rows++});return{file,rows,headers:meta.headers}}
async function loadSam(rec,file){if(!file)return{file:null,rows:0};let rows=0;const meta=await streamCsv(file,row=>{const m=fm(row),id=identity(m);if(!id)return;const r=ensure(rec,id);const nm=nameFrom(m);if(nm)r.names.add(nm);r.sources.add(file);if(truthy(first(m,['Small_Business_Flag']))||/SMALL BUSINESS/i.test(s(first(m,['Segment','Industry_Segment'])))){r.families.add('SBS_SAM');r.evidence.push('SAM:SMALL_BUSINESS')}rows++});return{file,rows,headers:meta.headers}}
