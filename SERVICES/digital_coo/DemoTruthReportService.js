...
class DemoTruthReportService {
  ...
  renderMarkdown(t) {
    const dealLines = t.revenue.deals.map(d=>`- ${d.company||d.name}: $${Number(d.value||0).toLocaleString()} | stage ${d.stage||'UNKNOWN'} | action ${d.action||'ΓÇö'}`).join('\n') || '- None';
