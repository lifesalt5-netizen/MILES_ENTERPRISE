...
class P2GCSalesQualificationService{
 qualify(input={}){
   ...
   let decision="GO";
   if(mandatoryMissing.length){decision=teamingCanMitigate?"TEAMING_REQUIRED":"NO_GO";}
   else if(input.riskFlags?.length) decision="GO_WITH_RISK";
   const result={ok:true,status:"QUALIFIED",generatedAt:now(),opportunityId:input.opportunityId||null,title:input.title||null,requirements,mandatoryMissing,riskFlags:arr(input.riskFlags),teamingMitigations:arr(input.teamingMitigations),decision,primeAllowed:decision==="GO"||decision==="GO_WITH_RISK",proposalAuthorized:decision==="GO"||decision==="GO_WITH_RISK",rules:{mandatoryQualificationBeforePrime:true,noUnsupportedExperienceClaims:true,noUnsupportedReferences:true,noAutomaticSubmission:true}};
 ...
 buildProposalPackage(input={}){
   ...
   if(!qualification.proposalAuthorized)return{ok:false,status:"BLOCKED_BY_QUALIFICATION",decision:qualification.decision,mandatoryMissing:qualification.mandatoryMissing};
 ...
 healthCheck(){return{ok:true,status:"HEALTHY",service:"P2GC_SALES_QUALIFICATION",generatedAt:now(),capabilities:["GO_NO_GO","QUALIFICATION_GATE","TEAMING_ROUTE","PROPOSAL_PACKAGE_DRAFT","COMPLIANCE_MATRIX"],externalSubmissionEnabled:false};}
}
