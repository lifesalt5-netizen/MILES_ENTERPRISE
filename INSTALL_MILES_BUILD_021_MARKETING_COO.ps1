# INSTALL_MILES_BUILD_021_MARKETING_COO.ps1
# Complete replacement of MarketingProvider only.
# Reuses existing InstantlyCOOService and authoritative runtime.
...
$BackupRoot =
    Join-Path $Root "_BACKUPS\BUILD_021_$Stamp"

$ReportDir =
    Join-Path $Root "DATA\build_021"
...
if (-not (Test-Path $Source)) {
    throw "Missing authoritative MarketingProvider: $Source"
}
...
'@ | Set-Content `
    -Path ".\PROVIDERS\providers\MarketingProvider.js" `
    -Encoding UTF8
...
'@ | Set-Content `
    -Path ".\TESTS\Test_Build021_MarketingCOO.js" `
    -Encoding UTF8

Write-Host ""
Write-Host "=== BUILD 021 SYNTAX VALIDATION ==="
...
$Files = @(
    ...
    ".\TESTS\Test_Build021_MarketingCOO.js"
)
...
Write-Host ""
Write-Host "=== BUILD 021 AUTOMATED TESTS ==="

$Output =
    & node ".\TESTS\Test_Build021_MarketingCOO.js" 2>&1
...
$Report =
    Join-Path `
        $ReportDir `
        "build_021_test_$Stamp.txt"

$Output |
    Tee-Object -FilePath $Report

if ($ExitCode -ne 0) {
    throw "Build 021 tests failed. Restore from $BackupRoot"
}
...
$Manifest = [ordered]@{
    ...
    capabilities = @(
        ...
        "Protected domain and inbox enforcement",
        "Segment inventory",
        "Lead upload readiness",
        "Deliverability recommendations"
    )
    safety = @(
        "Read-only operation",
        "No campaign pause or resume",
        "No automatic lead upload",
        "No account configuration changes",
        "pathways2gc.com remains protected",
        "info@pathways2gc.com remains protected"
    )
    report = $Report
}
...
$Manifest |
    ...
    Set-Content `
        -Path (
          Join-Path `
            $ReportDir `
            "build_021_manifest_$Stamp.json"
        ) `
        -Encoding UTF8

Write-Host ""
Write-Host "============================================================"
Write-Host "BUILD 021 MARKETING COO INSTALLED AND VERIFIED"
Write-Host "============================================================"
