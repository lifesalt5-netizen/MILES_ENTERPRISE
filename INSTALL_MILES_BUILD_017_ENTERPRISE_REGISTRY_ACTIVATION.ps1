# INSTALL_MILES_BUILD_017_ENTERPRISE_REGISTRY_ACTIVATION.ps1
...
#
# Uses existing authoritative registry infrastructure:
#   EnterpriseCapabilityRegistryService
#   ProviderAuthorityRegistryService
#   ProviderCapabilityBindingService
...
$BackupRoot = Join-Path $Root "_BACKUPS\BUILD_017_ENTERPRISE_$Stamp"
$TestDir = Join-Path $Root "TESTS"
$ReportDir = Join-Path $Root "DATA\build_017_enterprise"
...
Write-Host ""
Write-Host "============================================================"
Write-Host "MILES BUILD 017 - ENTERPRISE REGISTRY ACTIVATION"
...
class ProviderRouterService {
  ...
  authorityFor(providerName, action) {
    const providerKey =
      this.providerKeys[providerName] || null;

    const operation =
      this.actionOperations[providerName]?.[
        action
      ] || null;
  ...
  async invokeProvider(
    ...
    if (
      normalizedAction &&
      typeof provider[normalizedAction]
        === "function"
    ) {
      return provider[normalizedAction](
        task
      );
    }

    if (
      typeof provider.executeTask
        === "function"
    ) {
    ...
    throw new Error(
      `Provider exposes no executable action: ${
        normalizedAction || "unspecified"
      }`
    );
  }
  ...
  async executeProviderTask(task = {}) {
    ...
    try {
      ...
      return {
        ...
        actionInvoked:
          typeof provider[requestedAction]
            === "function"
            ? requestedAction
            : (
                typeof provider.executeTask
        ...
        taskId: task.id || null,
        workPackageId:
          payload.workPackageId || null,
        objective:
          payload.objective || null,
        capability:
          payload.capability || null,
        assignedTo:
          payload.assignedTo || "MILES",
        department:
  ...
  status() {
    ...
    return {
      ...
      providerAuthority: {
        ...
        summary:
          authorityRegistry.summary || null
      },
      capabilityBindings: {
        ok: bindingRegistry.ok,
        summary:
          bindingRegistry.summary || null
      }
    };
  }
}
...
$TestPath = Join-Path $TestDir "Test_Build017_EnterpriseRegistryActivation.js"
...
$SyntaxFiles = @(
    ...
    ".\TESTS\Test_Build017_EnterpriseRegistryActivation.js"
)
...
$TestReport = Join-Path $ReportDir "build_017_enterprise_test_$Stamp.txt"

$TestOutput |
    Tee-Object -FilePath $TestReport

if ($TestExit -ne 0) {
    throw "Build 017 automated tests failed. Restore from: $BackupRoot"
}

$Manifest = [ordered]@{
    ok = $true
    build = "017"
    name = "Enterprise Registry Activation"
...
$ManifestPath =
    Join-Path $ReportDir "build_017_enterprise_manifest_$Stamp.json"
...
Write-Host ""
Write-Host "============================================================"
Write-Host "BUILD 017 ENTERPRISE REGISTRY ACTIVATION COMPLETE"
Write-Host "============================================================"
