...
class AutonomousCOOLoopService {
  ...
  queueAuthorizedWorkflows() {
    ...
    for (const item of authorized) {
      ...
      let createdWorkPackageId = null;
      let createdWorkflowResult = null;
      let createdQueuedTasks = 0;
      ...
      try {
        ...
        workflowCreated =
          ...
          ["QUEUED", "COMPLETED", "IN_PROGRESS"]
            .includes(String(workflowResult?.status || "").toUpperCase());
        createdWorkPackageId = workPackageId;
        createdWorkflowResult = workflowResult;
        createdQueuedTasks = queuedTasks;
        ...
        if (!persisted || persisted.status !== "Queued") {
          if (workflowCreated) {
            console.warn(
              `[D015] Workflow ${workPackageId || "UNKNOWN"} was created for ${item.id}, but WorkQueue verification did not return Queued. Preserving the successful workflow and deferring reconciliation.`
            );
            ...
            results.push({
              ...
              message:
                "Workflow created successfully; WorkQueue reconciliation deferred."
            });
          ...
          throw new Error(
            `Failed to persist work item ${item.id} as Queued before any workflow was confirmed.`
          );
        }
        ...
        if (workflowCreated) {
          console.warn(
            `[D015] Workflow creation succeeded for ${item?.id || "UNKNOWN"}; suppressing false Failed status after persistence error.`
          );
          ...
          this.persistQueuedState(item.id, {
            ...
            queuedAt: now(),
            workflowStatus:
              createdWorkflowResult?.status || "QUEUED",
            workPackageId: createdWorkPackageId,
            workflowResult: createdWorkflowResult,
            sourceWorkItemId: item.id,
          ...
          results.push({
            ...
            workPackageId: createdWorkPackageId,
            queuedTasks: createdQueuedTasks,
            errorSuppressed: err.message
          });
        } else {
          if (
            item &&
  ...
  persistQueuedState(id, metadata = {}) {
    ...
    for (let attempt = 1; attempt <= 3; attempt++) {
      try {
        if (typeof this.workQueue.markQueued === "function") {
          updated = this.workQueue.markQueued(id, {
            ...metadata,
            persistenceAttempt: attempt,
            persistenceConfirmedAt: now()
          });
        } else if (typeof this.workQueue.updateStatus === "function") {
      ...
      if (attempt < 3) {
        const waitUntil = Date.now() + attempt * 75;
    ...
    if (lastError) {
      console.warn(
        `[D015] WorkQueue persistence warning for ${id}: ${lastError.message}`
      );
    }
