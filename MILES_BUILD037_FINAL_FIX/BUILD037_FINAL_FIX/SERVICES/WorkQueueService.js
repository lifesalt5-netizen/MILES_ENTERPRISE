...
class WorkQueueService {
    constructor(options = {}) {
        ...
        this.lockTimeoutMs =
            Number(options.lockTimeoutMs || 10000);

        this.staleLockMs =
            Number(options.staleLockMs || 30000);
    ...
    acquireQueueLock() {
        ...
        throw new Error(
            `WorkQueue lock timeout after ${this.lockTimeoutMs}ms: ${this.lockPath}`
        );
    }
    ...
    readQueueDocument() {
        ...
        return {
            metadata: parsed?.metadata || fallback.metadata,
    ...
    mergeQueueItems(diskItems = [], localItems = []) {
        ...
        for (const localItem of localItems || []) {
            ...
            const diskItem = merged.get(localItem.id);
            ...
            const diskTime = this.itemTimestamp(diskItem);
            const localTime = this.itemTimestamp(localItem);
            ...
            if (localTime >= diskTime) {
                merged.set(localItem.id, {
                    ...
                    lifecycle:
                        Array.isArray(localItem.lifecycle) &&
                        localItem.lifecycle.length >=
                            (Array.isArray(diskItem.lifecycle)
                                ? diskItem.lifecycle.length
                                : 0)
                            ? localItem.lifecycle
                            : diskItem.lifecycle
                });
    ...
    save(options = {}) {
        ...
        return this.withQueueLock(() => {
            const disk = fs.existsSync(this.queuePath)
                ? this.readQueueDocument()
                : { metadata: {}, items: [] };

            if (!replace) {
                this.queue = this.mergeQueueItems(
                    disk.items,
                    this.queue
                );
            }
    ...
    updateStatus(id, status, metadata = {}, note = "") {
        ...
        return this.withQueueLock(() => {
            const document = this.readQueueDocument();
            const items = document.items || [];
            const item = items.find(row => row && row.id === id);

            if (!item) {
                this.queue = items;
                this.metadata = document.metadata || this.metadata;
                return null;
            }
