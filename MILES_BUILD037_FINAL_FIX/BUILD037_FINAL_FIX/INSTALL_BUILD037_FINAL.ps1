...
$BackupRoot = Join-Path $MilesRoot "_BACKUPS\BUILD_037_FINAL_$Stamp"

Write-Host ""
Write-Host "MILES ENTERPRISE BUILD 037 FINAL FIX"
...
$Files = @(
    ...
    "TESTS\Test_Build037_FinalWorkflowPersistence.js"
)
...
node --check .\TESTS\Test_Build037_FinalWorkflowPersistence.js
if ($LASTEXITCODE -ne 0) { throw "Build 037 test syntax check failed." }

node .\TESTS\Test_Build037_FinalWorkflowPersistence.js
if ($LASTEXITCODE -ne 0) { throw "Build 037 regression test failed." }

Write-Host ""
Write-Host "BUILD 037 FINAL FIX INSTALLED AND VALIDATED"
Write-Host "Backup created at: $BackupRoot"
Write-Host ""
Write-Host "Start MILES with:"
