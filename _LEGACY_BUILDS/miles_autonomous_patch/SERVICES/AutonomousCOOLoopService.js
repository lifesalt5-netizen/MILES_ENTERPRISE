...
class AutonomousCOOLoopService {
  ...
  buildUniversalHealth(executiveState = {}) {
    ...
    const taskStatus = taskQueue && typeof taskQueue.getStatus === "function"
      ? taskQueue.getStatus()
    ...
    const runtimeHealth = {
      ...
      exceptions: taskStatus.failed > 0 ? [{ type: "TaskFailures", severity: "Warning", message: `${taskStatus.failed} failed runtime task(s) detected.` }] : [],
      recommendations: taskStatus.failed > 0 ? ["Review failed task details and queue safe repair work."] : ["Runtime is operational."],
      metrics: taskStatus
    };
    ...
    const workQueueHealth = {
      ...
      lastSuccess: now(),
      lastFailure: null,
      exceptions: queueStats.escalations > 0 ? [{ type: "Escalations", severity: "Warning", message: `${queueStats.escalations} Kevin-level escalation(s) open.` }] : [],
      recommendations: queueStats.escalations > 0 ? ["Review CEO approval queue and resolve blocked decisions."] : ["Work queue has no Kevin-level blocker."],
      metrics: queueStats
    };

    const systems = [runtimeHealth, workQueueHealth, ...providerHealth];
    const averageScore = systems.length
      ? Math.round(systems.reduce((sum, item) => sum + item.score, 0) / systems.length)
      : 0;
    ...
    return {
      ...
      overallStatus: averageScore >= 90 ? "Healthy" : averageScore >= 70 ? "Watch" : "Critical",
  ...
  buildMissionPlan(executiveState = {}, health = {}, cycleId = null) {
    ...
    for (const system of health.systems || []) {
      if (system.risk === "HIGH") {
        priorities.push(this.missionItem({
          priority: 1,
          area: system.area,
          title: `Repair high-risk ${system.area} condition`,
          reason: `${system.area} health score is ${system.score}.`,
          recommendedAction: (system.recommendations || [])[0] || `Investigate ${system.area} immediately.`,
          expectedImpact: "Protects operating continuity and removes an autonomy blocker.",
          requiresKevin: false,
      ...
      if (system.risk === "MEDIUM") {
        priorities.push(this.missionItem({
          ...
          recommendedAction: (system.recommendations || [])[0] || `Audit ${system.area} and recommend next action.`,
          expectedImpact: "Prevents small operational issues from becoming business blockers.",
          requiresKevin: false,
          relatedProvider: system.area,
          metadata: { health: system }
        }));
    ...
    if ((marketing.totalCampaigns || 0) > 0 && (marketing.activeCampaigns || 0) < 2) {
      priorities.push(this.missionItem({
        ...
        recommendedAction: "Audit paused Instantly campaigns, deliverability risk, inbox capacity, and safe resume options.",
    ...
    if ((orion.contractors || 0) > 0 && (orion.opportunities || 0) > 0) {
      priorities.push(this.missionItem({
        ...
        reason: "ORION has usable contractor and opportunity data and should remain current.",
        recommendedAction: "Audit ORION health, counts, recommendations, personas, and missing intelligence layers.",
        expectedImpact: "Protects demo readiness, opportunity intelligence, and contractor recommendations.",
        requiresKevin: false,
    ...
    for (const exception of exceptions) {
      ...
      priorities.push(this.missionItem({
        ...
        title: `${severity} exception: ${exception.type || exception.provider || "Unknown"}`,
        reason: exception.message || "Provider reported an exception.",
        recommendedAction: severity === "Critical" ? "Investigate immediately and repair if within governance." : "Investigate and monitor.",
        expectedImpact: severity === "Critical" ? "Prevents interruption to autonomous operations." : "Reduces operational risk.",
        requiresKevin: severity === "Critical" && /delete|price|contract|legal|publish|dns/i.test(exception.message || ""),
    ...
    for (const recommendation of recommendations.slice(0, 10)) {
      priorities.push(this.missionItem({
        priority: 3,
        area: "Executive",
        title: `Act on recommendation: ${String(recommendation).slice(0, 80)}`,
        reason: "Provider recommendation generated during executive intelligence refresh.",
        recommendedAction: String(recommendation),
        expectedImpact: "Improves operations based on live provider findings.",
        requiresKevin: false,
    ...
    if (priorities.length === 0) {
      priorities.push(this.missionItem({
        ...
        reason: "No urgent issues detected; MILES should continue monitoring and verification.",
        recommendedAction: "Refresh executive state, verify providers, and prepare next operating brief.",
    ...
    const deduped = this.deduplicateMissionItems(priorities)
      .sort((a, b) => a.priority - b.priority || b.businessImpactScore - a.businessImpactScore)
      .slice(0, 12);
    ...
    return {
      ...
      cycleId,
      generatedAt: now(),
      objective: "Move MILES closer to autonomous Digital COO operation for P2GC today.",
      priorities: deduped,
  ...
  missionItem(input = {}) {
    ...
    return {
      ...
      recommendedAction: input.recommendedAction || "Investigate and recommend next action.",
      ...
      businessImpactScore,
      automationConfidence: input.automationConfidence || (input.requiresKevin ? "MEDIUM" : "HIGH"),
  ...
  classifyRepair(system = {}, exception = {}) {
    const text = `${system.area || ""} ${exception.type || ""} ${exception.message || ""}`.toLowerCase();

    if (/delete|pricing|price|contract|legal|publish|dns|domain|payment|hire/.test(text)) {
      return {
        safeAutonomous: false,
        requiresKevin: true,
        recommendedRepair: "Prepare diagnosis and request CEO approval before action.",
        verification: "Verify CEO approval is recorded before any protected action.",
    ...
    if (/api|connect|key|login|session|browser/.test(text)) {
      return {
        safeAutonomous: true,
        requiresKevin: false,
        recommendedRepair: "Refresh provider state, retry connector/browser session, and record diagnostics.",
        verification: "Provider refresh succeeds or a clear escalation is recorded.",
        rollback: "Revert to previous cached state and preserve failure diagnostics."
      };
    }
    ...
    if (/campaign|paused|active|outbound|instantly|lead/.test(text)) {
      return {
        safeAutonomous: true,
        requiresKevin: false,
        recommendedRepair: "Audit campaign health, deliverability, inbox assignment, lead count, and safe next action.",
        verification: "Executive campaign report contains status, risk, and next action for each campaign.",
    ...
    return {
      safeAutonomous: true,
      requiresKevin: false,
      recommendedRepair: "Investigate issue, generate repair recommendation, and verify result.",
      verification: "Issue is resolved, downgraded, or escalated with evidence.",
  ...
  buildCapabilityBacklog(executiveState = {}, health = {}, cycleId = null) {
    const existingCapabilities = new Set(
      (executiveState.providers || []).map(p => String(p.provider || "").toLowerCase())
    );
    ...
    const targetCapabilities = [
      {
        ...
        removesManualWork: ["homepage audit", "CTA review", "form checks", "SEO checks", "conversion recommendations"],
        recommendedAction: "Add website provider and browser worker that audits homepage, services, forms, SEO, and conversion queue."
      },
      {
        area: "LinkedIn COO",
        ...
        businessImpact: "High",
        removesManualWork: ["profile audit", "company page audit", "post generation", "comment drafting", "engagement review"],
        recommendedAction: "Add LinkedIn provider and browser worker for audit, content queue, and governed posting."
      },
      {
        area: "Sales COO",
        providerKey: "sales",
        ...
        businessImpact: "Very High",
        removesManualWork: ["pipeline review", "follow-up reminders", "proposal status checks", "call prep", "revenue forecast"],
      ...
      },
      {
        ...
        businessImpact: "High",
        removesManualWork: ["SAM checks", "GSA checks", "VA checks", "RFI monitoring", "source stale checks"],
        recommendedAction: "Add source monitors for SAM, GSA, VA, forecasts, RFIs, and Sources Sought."
      },
      {
        ...
        businessImpact: "Very High",
        removesManualWork: ["runtime log review", "failed task diagnosis", "regression tests", "safe rollback", "capability backlog"],
        recommendedAction: "Add regression runner, repair evidence files, and guarded engineering backlog execution."
      }
    ];
  ...
  createMissionWork(mission = {}, repairPlan = {}, backlog = {}) {
    ...
    for (const item of mission.priorities || []) {
      created.push(this.workQueue.createWorkItem({
    ...
    for (const repair of repairPlan.repairs || []) {
      created.push(this.workQueue.createWorkItem({
        ...
        reason: "Autonomous repair candidate detected during COO health check.",
        source: "AutonomousRepairQueue",
        owner: "Miles",
        requiresKevin: repair.requiresKevin,
        recommendedAction: repair.recommendedRepair,
        expectedImpact: "Restores or protects autonomous operating capability.",
    ...
    for (const gap of (backlog.gaps || []).slice(0, 3)) {
      created.push(this.workQueue.createWorkItem({
        ...
        area: "Engineering",
        title: gap.title,
        description: `Missing capability: ${gap.area}. Removes manual work: ${gap.removesManualWork.join(", ")}.`,
    ...
    for (const item of authorized) {
      try {
        ...
        const context = {
          ...
          runtimeCycleId: item.metadata?.missionItem?.metadata?.cycleId || null
        };
  ...
  async runExecutionPasses() {
    ...
    for (let i = 0; i < this.maxExecutionPasses; i++) {
      const executionService = this.executionService || require("./ExecutionService");
      const result = await executionService.runNext();
      results.push({ pass: i + 1, result });
  ...
  scoreAutonomy(health = {}, mission = {}, repairPlan = {}, backlog = {}, learning = {}) {
    ...
    const govern = mission.escalationCount >= 0 && repairPlan.awaitingApproval >= 0 ? 78 : 45;

    const scores = { observe, understand, decide, execute, verify, learn, improve, govern };
    const overall = Math.round(Object.values(scores).reduce((sum, value) => sum + value, 0) / Object.values(scores).length);

    return {
      ok: true,
      level:
        overall >= 80 ? "BASIC_AUTONOMOUS_COO" :
        overall >= 65 ? "ASSISTED_AUTONOMOUS_COO" :
        "OPERATOR_MODE",
      overall,
      scores,
      interpretation:
  ...
  toMarkdown(result = {}) {
    ...
    for (const item of result.mission?.priorities || []) {
      lines.push(`- P${item.priority} — ${item.area}: ${item.title}`);
    ...
    for (const gap of result.capabilityBacklog?.gaps || []) {
      lines.push(`- P${gap.priority} — ${gap.area}: ${gap.title}`);
    }
