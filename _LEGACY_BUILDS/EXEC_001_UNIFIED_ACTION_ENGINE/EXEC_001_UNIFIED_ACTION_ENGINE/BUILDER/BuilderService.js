 ...
 *
 * Preserves existing builder actions and adds:
 * - ACTION_ENGINE
 * - PROVIDER_REGISTRY
 * - ACTION_HISTORY
 * - ACTION_AUDIT
...
class BuilderService {
    ...
    async execute(task = {}) {
        ...
        const commands = {
            ...
            DASHBOARD_SERVER: () =>
                require("../SERVICES/DashboardServerService").run(task),

            SELF_LEARNING: () =>
                require("../SERVICES/SelfLearningService").run(task),

            PROVIDER_REGISTRY: () =>
                require("../SERVICES/ProviderRegistryService").run(task),

            ACTION_ENGINE: () =>
                require("../SERVICES/ActionEngineService").run(task),

            ACTION_HISTORY: () =>
                require("../SERVICES/ActionHistoryService").run(task),

            ACTION_AUDIT: () =>
                require("../SERVICES/ActionAuditService").run(task)
        };
