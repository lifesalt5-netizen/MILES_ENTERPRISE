...
assert.deepStrictEqual(Object.keys(bridge.JOBS).sort(), [
  ...
  'P2GC_FEDERAL_GROWTH_REVIEW_DEPLOY',
...
assert.deepStrictEqual(bridge.JOBS.P2GC_FEDERAL_GROWTH_REVIEW_DEPLOY, ['node', ['SCRIPTS/DeployP2GCFederalGrowthReview.js']]);
...
assert.strictEqual(bridge.validateDirective({id:'x',enabled:true,job:'P2GC_FEDERAL_GROWTH_REVIEW_DEPLOY'}).ok, true);
...
assert(src.includes("P2GC_FEDERAL_GROWTH_REVIEW_DEPLOY: ['node', ['SCRIPTS/DeployP2GCFederalGrowthReview.js']]"));
