...
assert(src.includes('CALENDLY_DIRECT_PIPELINE_READ_FAILED'));
assert(src.includes('RESTORE_CALENDLY_DIRECT_PIPELINE_READ'));
assert(src.includes("Calendly direct pipeline error: ${calendlyPipelineError || 'none'}"));
assert(src.includes('!calendlyPipelineError && combinedMeetingEvidence'), 'Direct Calendly read failures must keep acceptance fail-closed');
