...
assert.strictEqual(state.stageState.PLAYBACK_TRACKING.status, 'PENDING', 'play alone must not satisfy the 75% meaningful-completion threshold');
state = svc.recordEngagement(review.reviewId, 'VIDEO_50');
assert.strictEqual(state.stageState.PLAYBACK_TRACKING.status, 'PENDING', '50% playback must remain below the contract threshold');
