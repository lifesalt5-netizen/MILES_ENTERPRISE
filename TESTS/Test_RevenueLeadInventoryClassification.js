...
(async function run() {
  ...
  fs.writeFileSync(path.join(sources, "SETASIDE_WOSB_EMAIL_READY.csv"), "Email,Segment\nemailready@example.com,WOSB\n", "utf8");
  fs.writeFileSync(path.join(sources, "INBOX_STATUS_MASTER.csv"), "Email\nsender@example.com\n", "utf8");
  ...
  await test("only lead CSV names are discovered", async () => assert.strictEqual(report.summary.sourceFilesDiscovered, 3));
  await test("verified file classifies good email", async () => assert.strictEqual(report.summary.verified, 1));
  await test("unknown and email-ready-only evidence remains pending", async () => assert.strictEqual(report.summary.pendingVerification, 2));
  ...
  await test("unique email count is conserved", async () => assert.strictEqual(report.summary.uniqueEmails, 5));
  await test("conservation validation passes", async () => assert.strictEqual(report.conservation.ok, true));
  await test("contact occurrences preserve overlaps", async () => assert.strictEqual(report.summary.contactOccurrences, 6));
