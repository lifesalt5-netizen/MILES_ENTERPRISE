...
class FakeWorkQueue {
  ...
  markCompleted(id, metadata = {}) { return this.updateStatus(id, "Completed", metadata); }
  markAwaitingApproval(id, metadata = {}) { return this.updateStatus(id, "Awaiting Approval", metadata); }
}
...
function workflow(taskIds, workPackageId) {
  return { workflowResult: { workPackageId, queuedTasks: taskIds.map(id => ({ id })) }, workPackageId };
}
