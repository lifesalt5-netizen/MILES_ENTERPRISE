...
(async function run() {
  ...
  const routes = [
    ...
    ["VOSB", 336, "c-vo", []],
    ["WOSB", 643, "c-wo", []],
    ["SBS", 194, "c-sbs", ["kevin@pathwaysgov.com"]],
    ["Unclassified", 2, null, []]
  ].map(([route, verifiedLeads, currentCampaignId, existingInboxes]) => ({
    route, verifiedLeads, currentCampaignId, existingInboxes, proposedCampaignName: route + " - Verified"
  }));
  fs.writeFileSync(planPath, JSON.stringify({
    ok: true, status: "ALL_SEGMENT_CONFIGURATION_PLANNED", configurationFingerprint: "A".repeat(64),
    summary: { verifiedLeads: 8578, uniqueEmails: 8578, unclassifiedLeads: 2 },
    conservation: { ok: true }, globalDeduplication: { ok: true }, routes
  }), "utf8");

  const created = [], updated = [], paused = [];
  const service = new Service({
    rootDir: root, planPath, outputRoot,
    outputPath: path.join(outputRoot, "manifest.json"),
  ...
  await test("provider write scope is constrained", async () => assert.strictEqual(report.providerWriteScope, "CREATE_2_CAMPAIGNS_PAUSE_10_CAMPAIGNS_ASSIGN_9_INBOXES"));
  ...
  const dry = new Service({
    rootDir: root, planPath, outputRoot: path.join(root, "dry"),
    createProvider: async () => ({ dryRun: true, mutationExecuted: false }),
    updateProvider: async () => ({ id: "x" }),
    pauseProvider: async () => ({ id: "x" })
  });
