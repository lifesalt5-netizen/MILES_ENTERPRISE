...
class EnterpriseStore {
  constructor() {
    ...
    try {
      ...
      console.error("[EnterpriseStore] SQLite initialization failed; using JSON fallback.");
      console.error(err);
      this.mode = "json";
      this.db = null;
      this.initJson();
  ...
  initJson() {
    for (const table of [
      ...
      "marketingCapacityPlans","marketingCapacityAllocations","marketingUploadQueue","marketingUploadQueueRuns",
      "marketingLeadUploadHistory","marketingRecommendations"
    ]) {
...
return (this.json.approvals || [])
  ...
  createMarketingRecommendation(item = {}) {


    const row = {

      id:item.id || this.id("MARKETING_REC"),
      ...
      recommendedUpload:Number(item.recommendedUpload || 0),
    ...
    this.db.prepare(
`
INSERT OR REPLACE INTO marketing_recommendations
(
id,
segmentId,
segmentName,
...
`
  ...
  getMarketingRecommendations(){
    ...
    return this.db.prepare(
      "SELECT * FROM marketing_recommendations ORDER BY priority DESC"
    ).all();
  ...
  ensureMarketingRecommendations(){
