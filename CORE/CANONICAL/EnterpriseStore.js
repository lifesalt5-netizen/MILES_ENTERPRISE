  ...
  getPendingApprovals(department = null) {
    if (this.mode === "sqlite") {
      const rows = department
        ? this.db.prepare("SELECT * FROM approvals WHERE status='PENDING' AND department=? ORDER BY createdAt ASC").all(department)
        : this.db.prepare("SELECT * FROM approvals WHERE status='PENDING' ORDER BY createdAt ASC").all();

      return rows.map(x => Object.assign({}, x, { payload: parseJson(x.payload) }));
    }

    return (this.json.approvals || []).filter(x => x.status === "PENDING" && (!department || x.department === department));
  }
