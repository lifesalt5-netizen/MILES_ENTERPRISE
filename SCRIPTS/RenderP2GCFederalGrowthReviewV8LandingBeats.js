...
// V8 pacing-only wrapper around the approved V7 presentation.
// Keeps voice, narration-driven sync, intro, SLED-to-Fed mention, and core content unchanged.
// Only increases the landing beat between scenes so key points register before the next topic.
...
const GENERATED = path.join(__dirname, '_runtime_RenderP2GCFederalGrowthReviewV8LandingBeats.js');

function main() {
  let src = fs.readFileSync(SOURCE, 'utf8');

  src = src
    .replace("path.join(ROOT,'DATA','reusable_demo','v5')", "path.join(ROOT,'DATA','reusable_demo','v8')")
    .replace('P2GC_Federal_Growth_Review_Demo_V5.mp4', 'P2GC_Federal_Growth_Review_Demo_V8.mp4')
    .replace('latest_p2gc_v5_render.json', 'latest_p2gc_v8_render.json')
    .replace("const RATE='-8%';", "const RATE='-18%';")
    .replace('const BETWEEN_CHUNK_HOLD=0.10;', 'const BETWEEN_CHUNK_HOLD=0.14;')
    .replace('const BETWEEN_SCENE_HOLD=0.18;', 'const BETWEEN_SCENE_HOLD=0.80;')
    .replace(/V5_SPEECH_SYNC_RENDER/g, 'V8_LANDING_BEATS_RENDER')
    .replace(/Final V5 speech-synced MP4 created/g, 'Final V8 landing-beat speech-synced MP4 created')
    .replace(/FINAL_V5_/g, 'FINAL_V8_')
    .replace(/v5_concat\.txt/g, 'v8_concat.txt')
    .replace(/final_v5_mp4_created/g, 'final_v8_mp4_created');
  ...
  if (!src.includes(anchor)) throw new Error('V8_INJECTION_ANCHOR_NOT_FOUND');
  ...
  src = src
    ...
    .replace("status.goGreen=true;", "status.targetRuntimeSeconds=485;status.runtimeTargetWindowSeconds=[475,500];status.goGreen=actual>=475&&actual<=500;");
