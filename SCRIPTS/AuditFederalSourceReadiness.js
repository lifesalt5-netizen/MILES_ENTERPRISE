...
async function timedAsync(fn){const t=Date.now();try{return {ms:elapsed(t),result:await fn()};}catch(error){return {ms:elapsed(t),error:String(error?.stack||error?.message||error)};}}
function timedSync(fn){const t=Date.now();try{return {ms:elapsed(t),result:fn()};}catch(error){return {ms:elapsed(t),error:String(error?.stack||error?.message||error)};}}
...
async function diagnoseP2GC(rootDir,term='DeLune Corporation'){
  ...
  try{
    ...
    const samFallback=timedSync(()=>sam.build(term));
    const historicalFallback=timedSync(()=>historical.build(term,{samFallback:samFallback.result||null,orionFailure:base.result||null}));
    ...
    const stages={
      ...
      samFallback:{ms:samFallback.ms,...compactModel(samFallback.result),error:samFallback.error||null},
    ...
    const flat=[
      ['BASE_ORION',stages.baseOrion.ms],['SAM_FALLBACK',stages.samFallback.ms],['HISTORICAL_FALLBACK',stages.historicalFallback.ms],['CURRENT_SAM_BY_UEI',stages.currentSamByUei.ms],
  ...
  }finally{try{sam.close();}catch{} try{historical.close?.();}catch{}}
}
