...
async function main(){const rootDir=path.resolve(process.argv[2]||process.env.MILES_ROOT||path.resolve(__dirname,'..'));const result=await new Service({rootDir}).run();console.log(JSON.stringify(result,null,2));process.exitCode=result.ok?0:3;}
