...
function main() {
  ...
  // Keep URL visually clean while narration can say "slash schedule" naturally.
  ...
  const newKey = "function keyLine(text){let s=clean(text).replace(/pathways2gc\\.com\\s+slash\\s+schedule/ig,'pathways2gc.com/schedule');if(s.length<=165)return s;const cut=s.slice(0,165);const i=Math.max(cut.lastIndexOf(','),cut.lastIndexOf(';'),cut.lastIndexOf(' '));return cut.slice(0,i>90?i:165)+'…';}";
  ...
  const injected = "if(!Array.isArray(master.scenes)||master.scenes.length!==11)throw new Error('APPROVED_SCENE_COUNT_INVALID');" +
    ...
    "const scene2=master.scenes.find(s=>s.scene===2);if(scene2){scene2.narration=scene2.narration+\" Now, let’s take a look at a fictional company so you can see how we review a business, what we look for, why those findings matter, and how they can point to a practical growth path.\";}" +
