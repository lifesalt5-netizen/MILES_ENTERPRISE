...
async function main(){
  ...
  const inboxAudit=runJsonAudit('SCRIPTS/AuditPrimaryInboxCoverage.js',120000);const vidsAudit=runJsonAudit('SCRIPTS/AuditGoogleVidsEligibility.js',180000);const editorAudit=runJsonAudit('SCRIPTS/AuditGoogleVidsEditorAvatarEligibility.js',180000);const localAvatarAudit=runJsonAudit('SCRIPTS/AuditLocalAvatarRuntime.js',90000);const ingressAudit=runJsonAudit('SCRIPTS/AuditP2GCReviewPublicIngress.js',90000);const gatewayCandidateAudit=runJsonAudit('SCRIPTS/AuditP2GCFunnelGatewayCandidates.js',90000);const infoRoute=inboxAudit.result?.requiredInboxes?.find(x=>String(x.email||'').toLowerCase()==='info@pathways2gc.com')||null;
  const videoSummary={
    googleVidsAccess:vidsAudit.ok,
    googleVidsStatus:vidsAudit.result?.status||vidsAudit.result?.browser?.status||'UNKNOWN',
    googleEditorOk:editorAudit.ok,
    googleEditorStatus:editorAudit.result?.status||'UNKNOWN',
    googleEditorAccount:editorAudit.result?.selected?.email||null,
    googleAvatarVisible:editorAudit.result?.editor?.avatarVisible===true,
    localRuntimeAuditOk:localAvatarAudit.ok,
    localTalkingAvatarReady:localAvatarAudit.result?.recommendation?.localTalkingAvatarCandidate===true,
    localCpuCandidate:localAvatarAudit.result?.recommendation?.cpuOnlyCandidate===true,
    localGpuCandidate:localAvatarAudit.result?.recommendation?.gpuAcceleratedCandidate===true,
    ffmpegReady:localAvatarAudit.result?.ffmpeg?.available===true,
    ingressAuditOk:ingressAudit.ok,
    ingressConclusion:ingressAudit.result?.conclusion||'UNKNOWN',
    publicBaseConfigured:ingressAudit.result?.publicBaseUrl?.configured===true,
    tailscaleFunnelActive:ingressAudit.result?.tailscale?.funnel?.active===true,
    gatewayCandidateAuditOk:gatewayCandidateAudit.ok,
    gatewayCandidatePath:gatewayCandidateAudit.result?.bestCandidate?.path||null,
    gatewayCandidateScore:gatewayCandidateAudit.result?.bestCandidate?.score??null,
    gatewayCandidateHighConfidence:gatewayCandidateAudit.result?.highConfidence===true
  };
  videoSummary.selectedProvider=videoSummary.googleAvatarVisible?'GOOGLE_VIDS':videoSummary.localTalkingAvatarReady?'LOCAL_OPEN_SOURCE':'NONE_PROVEN';
  ...
  console.log(`PRIMARY_INBOX_AUDIT_OK=${inboxAudit.ok}`);console.log(`INFO_PATHWAYS2GC_PROVIDER_ROUTE=${infoRoute?.route||'UNKNOWN'}`);console.log(`INFO_PATHWAYS2GC_REGISTERED_GMAIL=${infoRoute?.registeredGmailAccount===true}`);console.log(`INFO_PATHWAYS2GC_IONOS_READABLE=${infoRoute?.ionosReadable===true}`);
  console.log(`GOOGLE_VIDS_ELIGIBILITY_AUDIT_OK=${vidsAudit.ok}`);console.log(`GOOGLE_VIDS_ELIGIBILITY_STATUS=${videoSummary.googleVidsStatus}`);
  console.log(`GOOGLE_VIDS_EDITOR_AUDIT_OK=${editorAudit.ok}`);console.log(`GOOGLE_VIDS_EDITOR_STATUS=${videoSummary.googleEditorStatus}`);console.log(`GOOGLE_VIDS_EDITOR_ACCOUNT=${videoSummary.googleEditorAccount||'UNKNOWN'}`);console.log(`GOOGLE_VIDS_EDITOR_AVATAR_VISIBLE=${videoSummary.googleAvatarVisible}`);
  console.log(`LOCAL_AVATAR_RUNTIME_AUDIT_OK=${localAvatarAudit.ok}`);console.log(`LOCAL_AVATAR_RUNTIME_CANDIDATE=${videoSummary.localTalkingAvatarReady}`);console.log(`LOCAL_AVATAR_GPU_CANDIDATE=${videoSummary.localGpuCandidate}`);console.log(`LOCAL_AVATAR_CPU_CANDIDATE=${videoSummary.localCpuCandidate}`);console.log(`LOCAL_AVATAR_FFMPEG_READY=${videoSummary.ffmpegReady}`);
  console.log(`P2GC_REVIEW_INGRESS_AUDIT_OK=${ingressAudit.ok}`);console.log(`P2GC_REVIEW_INGRESS_CONCLUSION=${videoSummary.ingressConclusion}`);console.log(`P2GC_REVIEW_PUBLIC_BASE_CONFIGURED=${videoSummary.publicBaseConfigured}`);console.log(`P2GC_REVIEW_TAILSCALE_FUNNEL_ACTIVE=${videoSummary.tailscaleFunnelActive}`);
  console.log(`P2GC_FUNNEL_GATEWAY_CANDIDATE_AUDIT_OK=${gatewayCandidateAudit.ok}`);console.log(`P2GC_FUNNEL_GATEWAY_CANDIDATE_PATH=${videoSummary.gatewayCandidatePath||'UNKNOWN'}`);console.log(`P2GC_FUNNEL_GATEWAY_CANDIDATE_SCORE=${videoSummary.gatewayCandidateScore??'UNKNOWN'}`);console.log(`P2GC_FUNNEL_GATEWAY_CANDIDATE_HIGH_CONFIDENCE=${videoSummary.gatewayCandidateHighConfidence}`);
  console.log(`P2GC_REVIEW_VIDEO_READINESS_SUMMARY=${JSON.stringify(videoSummary)}`);
  if(!inboxAudit.result&& (inboxAudit.stderr||inboxAudit.stdout))console.log(`PRIMARY_INBOX_AUDIT_DIAGNOSTIC=${JSON.stringify({stdout:inboxAudit.stdout,stderr:inboxAudit.stderr,exitCode:inboxAudit.exitCode})}`);
  if(!vidsAudit.result&& (vidsAudit.stderr||vidsAudit.stdout))console.log(`GOOGLE_VIDS_ELIGIBILITY_DIAGNOSTIC=${JSON.stringify({stdout:vidsAudit.stdout,stderr:vidsAudit.stderr,exitCode:vidsAudit.exitCode})}`);
  if(!editorAudit.result&& (editorAudit.stderr||editorAudit.stdout))console.log(`GOOGLE_VIDS_EDITOR_DIAGNOSTIC=${JSON.stringify({stdout:editorAudit.stdout,stderr:editorAudit.stderr,exitCode:editorAudit.exitCode})}`);
  if(!localAvatarAudit.result&& (localAvatarAudit.stderr||localAvatarAudit.stdout))console.log(`LOCAL_AVATAR_RUNTIME_DIAGNOSTIC=${JSON.stringify({stdout:localAvatarAudit.stdout,stderr:localAvatarAudit.stderr,exitCode:localAvatarAudit.exitCode})}`);
  if(!ingressAudit.result&& (ingressAudit.stderr||ingressAudit.stdout))console.log(`P2GC_REVIEW_INGRESS_DIAGNOSTIC=${JSON.stringify({stdout:ingressAudit.stdout,stderr:ingressAudit.stderr,exitCode:ingressAudit.exitCode})}`);
  if(!gatewayCandidateAudit.result&&(gatewayCandidateAudit.stderr||gatewayCandidateAudit.stdout))console.log(`P2GC_FUNNEL_GATEWAY_CANDIDATE_DIAGNOSTIC=${JSON.stringify({stdout:gatewayCandidateAudit.stdout,stderr:gatewayCandidateAudit.stderr,exitCode:gatewayCandidateAudit.exitCode})}`);
