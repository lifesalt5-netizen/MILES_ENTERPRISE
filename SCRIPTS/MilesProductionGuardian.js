...
const VERIFY_ONLY = process.argv.includes("--verify") || !REPAIR;
const EXPECTED_APPS = ["miles-worker", "miles-ui", "miles-dashboard", "miles-command-center"];
const MEMORY_WARN_MB = Number(process.env.MILES_WORKER_MEMORY_WARN_MB || 2048);
...
function reclaimStaleLock() {
  ...
  if (owner && processAlive(owner.pid)) {
    return { present: true, reclaimed: false, reason: "owner_alive", ownerPid: owner.pid };
  }
...
async function verify() {
  ...
  else if (worker && worker.memoryMB >= MEMORY_WARN_MB) warnings.push(`worker memory high ${worker.memoryMB} MB`);
...
async function main() {
  ...
  const report = { service: "MILES_PRODUCTION_GUARDIAN", startedAt: now(), mode: REPAIR ? "REPAIR" : "VERIFY", actions: [] };
  ...
  if (REPAIR) {
    ...
    sleep(5000);
    const killed = killForkChildren();
    report.actions.push({ action: "kill_orphan_pm2_forks", ok: true, pids: killed });
    sleep(1500);
  ...
  for (const app of report.verification.apps) {
    console.log(`${app.name.padEnd(22)} status=${app.status} pid=${app.pid} ramMB=${app.memoryMB} restarts=${app.restarts}`);
  }
