...
async function readCampaignLeads(campaignId){
  ...
  for(let page=0;page<1000;page++){
    const response=await instantly.listLeads({campaign:campaignId,limit:100,distinct_contacts:true,...(startingAfter?{starting_after:startingAfter}:{})});
    ...
    const lastEmail=liveEmail(x.items[x.items.length-1]); if(lastEmail) cursor=lastEmail;
