...
function Add-Result {
    ...
    $results.Add([pscustomobject]@{name=$Name;status=$Status;category=$Category;exitCode=$ExitCode;detail=$Detail;externalMutation=$Mutation}) | Out-Null
}
function Invoke-NodeCheck {
    ...
    & node --check $full *> $null; $ec=$LASTEXITCODE
...
function Invoke-NodeRun {
    ...
    $output=& node $full @Args 2>&1 | Out-String; $ec=$LASTEXITCODE
...
function Invoke-PowerShellRun {
    ...
    $full=Join-Path $Root $Path
    if(-not(Test-Path $full)){Add-Result $Name 'YELLOW' $Category 0 "MISSING:$Path";return}
    $output=& powershell.exe -NoProfile -ExecutionPolicy Bypass -File $full @Args 2>&1 | Out-String; $ec=$LASTEXITCODE
    $tail=if($output.Length -gt 3000){$output.Substring($output.Length-3000)}else{$output}
    Add-Result $Name ($(if($ec -eq 0){'GREEN'}else{'RED'})) $Category $ec $tail.Trim()
}

$syntaxFiles=@(
'SCRIPTS/AuditInstantlyCampaignScheduleGovernance.js','SCRIPTS/AuditInstantlySendWindowHistory.js','SCRIPTS/RepairInstantlyWeekdaySchedules.js','SCRIPTS/AUDIT_MILES_REVENUE_OPERATIONS.js','SERVICES/sales/P2GCSalesQualificationService.js','SERVICES/FederalPathwayScoreService.js','SERVICES/FederalPathwayScoreIntegratedService.js','SERVICES/revenue/RevenueWeightedCampaignScorecardService.js','SERVICES/revenue/QualifiedProspectNurtureService.js','SERVICES/revenue/P2GCAcquisitionV2CampaignService.js','SERVICES/revenue/P2GCAcquisitionV2ProspectEnrichmentService.js','SERVICES/revenue/P2GCWebsiteConversionAuditService.js','CONNECTORS/WEBSITE_B12/B12_CONTROLLED_PUBLISHER.js','RUN_FEDERAL_PATHWAY_SCORE_INTEGRATED.js','RUN_P2GC_REVENUE_SCORECARD.js','RUN_P2GC_QUALIFIED_NURTURE.js','RUN_P2GC_ACQUISITION_V2_PILOT.js','SCRIPTS/AuditProposalCommandReadiness.js')
foreach($f in $syntaxFiles){Invoke-NodeCheck "syntax:$f" $f 'STATIC'}

$tests=@(
'TESTS/Test_InstantlyCampaignScheduleGovernance.js','TESTS/Test_InstantlySendWindowHistory.js','TESTS/Test_P2GCSalesQualificationDecisionLabels.js','TESTS/Test_FederalPathwayScoreService.js','TESTS/Test_FederalPathwayScoreIntegratedService.js','TESTS/Test_RevenueWeightedCampaignScorecardService.js','TESTS/Test_QualifiedProspectNurtureService.js','TESTS/Test_P2GCAcquisitionV2CampaignService.js','TESTS/Test_P2GCAcquisitionV2ProspectEnrichmentService.js','TESTS/Test_P2GCB12ConversionManifest.js','TESTS/Test_P2GCWebsiteConversionAuditService.js','TESTS/E0030RevenueTruthGate.test.js','TESTS/meeting_pipeline_readonly_safety_test.js','TESTS/production_go_live_acceptance_audit_safety_test.js','TESTS/TestExecutiveDashboardData.js','TESTS/TestExecutiveMissionRouter.js','TESTS/TestExecutiveToEngineering.js','TESTS/TestExecutiveToEngineeringFlow.js')
...
$oldExec=$env:P2GC_ACQ_V2_EXECUTE; $oldActivate=$env:P2GC_ACQ_V2_ACTIVATE
try{
    $env:P2GC_ACQ_V2_EXECUTE='false';$env:P2GC_ACQ_V2_ACTIVATE='false'
...
}finally{$env:P2GC_ACQ_V2_EXECUTE=$oldExec;$env:P2GC_ACQ_V2_ACTIVATE=$oldActivate}
...
$result=[ordered]@{ok=($red -eq 0);status=if($red -eq 0){'PRE_FINAL_SOAK_READINESS_GREEN'}else{'PRE_FINAL_SOAK_READINESS_BLOCKED'};generatedAt=(Get-Date).ToUniversalTime().ToString('o');startedAt=$startedAt;gitHead=$head;gitBranch=$branch;executeInstantlyWeekdayRepairRequested=[bool]$ExecuteInstantlyWeekdayRepair;counts=@{green=$green;yellow=$yellow;red=$red;controlledMutations=$mutations;total=$results.Count};rules=@('No acquisition campaign creation/activation during readiness.','No B12 public publish during readiness.','Instantly weekday repair is the only allowed external mutation and only with the explicit switch.','Final 24-hour soak must begin only after all RED blockers are resolved and one release-candidate commit is frozen.');results=@($results)}
