...
async function main(){
  ...
  const commandLine=safeToken(gateway.owner?.json?.commandLine,220);
  ...
  const result={ok:true,service:'P2GC_REVIEW_PUBLIC_INGRESS_AUDIT',observedAt:new Date().toISOString(),publicBaseUrl:{configured:Boolean(publicBase),https:publicHttpsConfigured,value:publicBase||null},backend,backendAdmin:{...backendAdmin,privateReady:localAdminReady},gateway:{...gateway,alreadyRoutesReview:gatewayAlreadyRoutesReview,adminNotExposed,ownerName,commandLineCompact:commandLine},tailscale:{cliAvailable:Boolean(cli),path:tailscaleWhere.stdout||tailscaleExeWhere.stdout||null,status:{ok:status.ok,json:statusJson,stdout:statusJson?null:status.stdout,stderr:status.stderr},serve:{ok:serve.ok,active:serveActive,json:serveJson,stdout:serveJson?null:serve.stdout,stderr:serve.stderr},funnel:{ok:funnel.ok,active:funnelActive,json:funnelJson,stdout:funnelJson?null:funnel.stdout,stderr:funnel.stderr}},conclusion,safety:{readOnly:true,networkChanged:false,dnsChanged:false,funnelChanged:false,serveChanged:false,publicExposureCreated:false}};
