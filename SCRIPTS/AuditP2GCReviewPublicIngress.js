...
function request(port,pathname,timeout=8000){return new Promise(resolve=>{const req=http.get({host:'127.0.0.1',port,path:pathname,timeout},res=>{const chunks=[];res.on('data',c=>chunks.push(c));res.on('end',()=>{const body=Buffer.concat(chunks).toString('utf8');resolve({ok:res.statusCode>=200&&res.statusCode<500,statusCode:res.statusCode,headers:{contentType:res.headers['content-type']||null,cacheControl:res.headers['cache-control']||null,xRobotsTag:res.headers['x-robots-tag']||null,server:res.headers.server||null},bodyPreview:body.slice(0,2500),reviewMarker:/Personalized Federal Growth Review/i.test(body)});});});req.on('timeout',()=>req.destroy(new Error('TIMEOUT')));req.on('error',e=>resolve({ok:false,error:e.message}));});}
...
async function main(){
  ...
  const backend=await request(PORT,'/review/P2GC-FGR-INGRESS-PROBE');
  ...
  const gatewayAlreadyRoutesReview=gateway.reviewProbe.statusCode===200&&gateway.reviewProbe.reviewMarker===true;
  const adminNotExposed=gateway.adminProbe.statusCode===403||gateway.adminProbe.statusCode===404||gateway.adminProbe.ok===false;
  const result={ok:true,service:'P2GC_REVIEW_PUBLIC_INGRESS_AUDIT',observedAt:new Date().toISOString(),publicBaseUrl:{configured:Boolean(publicBase),https:publicHttpsConfigured,value:publicBase||null},backend,gateway:{...gateway,alreadyRoutesReview:gatewayAlreadyRoutesReview,adminNotExposed},tailscale:{cliAvailable:Boolean(cli),path:tailscaleWhere.stdout||tailscaleExeWhere.stdout||null,status:{ok:status.ok,json:statusJson,stdout:statusJson?null:status.stdout,stderr:status.stderr},serve:{ok:serve.ok,active:serveActive,json:serveJson,stdout:serveJson?null:serve.stdout,stderr:serve.stderr},funnel:{ok:funnel.ok,active:funnelActive,json:funnelJson,stdout:funnelJson?null:funnel.stdout,stderr:funnel.stderr}},conclusion:publicHttpsConfigured?'PUBLIC_HTTPS_BASE_CONFIGURED':gatewayAlreadyRoutesReview&&funnelActive?'EXISTING_FUNNEL_ALREADY_ROUTES_REVIEW':funnelActive?'TAILSCALE_FUNNEL_PRESENT_GATEWAY_8779_REVIEW_ROUTE_NOT_PROVEN':serveActive?'TAILSCALE_SERVE_PRESENT_PRIVATE_ONLY':'NO_PUBLIC_INGRESS_PROVEN',safety:{readOnly:true,networkChanged:false,dnsChanged:false,funnelChanged:false,serveChanged:false,publicExposureCreated:false}};
