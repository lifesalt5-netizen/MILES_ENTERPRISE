...
function windowsPortOwner(port){
  if(process.platform!=='win32')return{ok:false,status:'WINDOWS_ONLY'};
  const ps=`$c=Get-NetTCPConnection -LocalPort ${port} -State Listen -ErrorAction SilentlyContinue | Select-Object -First 1; if($c){$p=Get-CimInstance Win32_Process -Filter \"ProcessId=$($c.OwningProcess)\" -ErrorAction SilentlyContinue; [pscustomobject]@{port=${port};pid=$c.OwningProcess;parentPid=$p.ParentProcessId;process=$p.Name;executablePath=$p.ExecutablePath;commandLine=$p.CommandLine;creationDate=$p.CreationDate} | ConvertTo-Json -Compress}`;
  const r=run('powershell.exe',['-NoProfile','-NonInteractive','-Command',ps],15000);return{...r,json:parseJson(r.stdout)};
}
function windowsProcessChain(pid){
  if(process.platform!=='win32'||!Number(pid))return{ok:false,status:'WINDOWS_ONLY_OR_PID_MISSING',chain:[]};
  const ps=`$id=${Number(pid)};$out=@();for($i=0;$i -lt 5 -and $id -gt 0;$i++){ $p=Get-CimInstance Win32_Process -Filter \"ProcessId=$id\" -ErrorAction SilentlyContinue; if(-not $p){break}; $out += [pscustomobject]@{pid=$p.ProcessId;parentPid=$p.ParentProcessId;name=$p.Name;executablePath=$p.ExecutablePath;commandLine=$p.CommandLine;creationDate=$p.CreationDate}; $id=[int]$p.ParentProcessId }; $out | ConvertTo-Json -Compress`;
  const r=run('powershell.exe',['-NoProfile','-NonInteractive','-Command',ps],20000);const parsed=parseJson(r.stdout);return{...r,chain:Array.isArray(parsed)?parsed:(parsed?[parsed]:[])};
}
function windowsRunPyCandidates(){
  ...
  const ps=`$roots=@('${base}',$env:USERPROFILE,$env:ProgramData,$env:TEMP) | Where-Object {$_ -and (Test-Path -LiteralPath $_)} | Select-Object -Unique; $out=@(); foreach($r in $roots){$out += Get-ChildItem -LiteralPath $r -Filter run.py -File -Recurse -ErrorAction SilentlyContinue | Select-Object -First 25 -ExpandProperty FullName}; $out | Select-Object -Unique -First 50 | ConvertTo-Json -Compress`;
  const r=run('powershell.exe',['-NoProfile','-NonInteractive','-Command',ps],60000);
...
function chainTag(chain=[]){return chain.slice(0,4).map(p=>`${String(p.name||'UNKNOWN').replace(/[^A-Za-z0-9_.-]/g,'_')}:${pathTag(p.commandLine||'')}`).join('>');}
...
async function main(){
  ...
  const owner=windowsPortOwner(FUNNEL_GATEWAY_PORT);const gateway={port:FUNNEL_GATEWAY_PORT,owner,processChain:windowsProcessChain(owner?.json?.pid),runPyCandidates:windowsRunPyCandidates(),root:await request(FUNNEL_GATEWAY_PORT,'/'),health:await request(FUNNEL_GATEWAY_PORT,'/api/health'),reviewProbe:await request(FUNNEL_GATEWAY_PORT,'/review/P2GC-FGR-INGRESS-PROBE'),reviewHealth:await request(FUNNEL_GATEWAY_PORT,'/api/review/health'),adminProbe:await request(FUNNEL_GATEWAY_PORT,'/api/admin/review/health')};
  ...
  const commandLine=safeToken(gateway.owner?.json?.commandLine,220);const gatewayScript=scriptTag(gateway.owner?.json?.commandLine);const candidates=gateway.runPyCandidates?.paths||[];const candidateTag=candidates.length===1?pathTag(candidates[0]):`CANDIDATES_${candidates.length}`;const parents=chainTag(gateway.processChain?.chain||[]);
  ...
  const result={ok:true,service:'P2GC_REVIEW_PUBLIC_INGRESS_AUDIT',observedAt:new Date().toISOString(),publicBaseUrl:{configured:Boolean(publicBase),https:publicHttpsConfigured,value:publicBase||null},backend,backendAdmin:{...backendAdmin,privateReady:localAdminReady},gateway:{...gateway,alreadyRoutesReview:gatewayAlreadyRoutesReview,adminNotExposed,ownerName,gatewayScript,commandLineCompact:commandLine,processChainCompact:parents},tailscale:{cliAvailable:Boolean(cli),path:tailscaleWhere.stdout||tailscaleExeWhere.stdout||null,status:{ok:status.ok,json:statusJson,stdout:statusJson?null:status.stdout,stderr:status.stderr},serve:{ok:serve.ok,active:serveActive,json:serveJson,stdout:serveJson?null:serve.stdout,stderr:serve.stderr},funnel:{ok:funnel.ok,active:funnelActive,json:funnelJson,stdout:funnelJson?null:funnel.stdout,stderr:funnel.stderr}},conclusion,safety:{readOnly:true,networkChanged:false,dnsChanged:false,funnelChanged:false,serveChanged:false,publicExposureCreated:false}};
