...
try{
  ...
  const evidence={ok:true,twin:"MONICA",mode:"STANDALONE_DISCOVERY_ONLY",installedAt:new Date().toISOString(),root,steps};
  fs.writeFileSync(path.join(outDir,"MONICA_INSTALL_ACCEPTANCE.json"),JSON.stringify(evidence,null,2)+"\n","utf8");
  console.log(JSON.stringify({ok:true,twin:"MONICA",acceptance:path.join(outDir,"MONICA_INSTALL_ACCEPTANCE.json")},null,2));
}catch(err){
