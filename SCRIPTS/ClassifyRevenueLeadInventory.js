...
function main() {
  const result = new Service().classifyInventory(parseArguments(process.argv.slice(2)));
  console.log(JSON.stringify(result, null, 2));
  if (result.ok !== true) process.exitCode = 1;
  else if (result.mode === "PLAN_ONLY") console.log("\nPLAN ONLY. Re-run with --apply to classify all authoritative lead CSV evidence.");
}
