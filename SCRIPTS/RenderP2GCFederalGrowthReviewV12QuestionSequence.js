...
const TEMP_WRAPPER = path.join(__dirname, '_runtime_BuildP2GCFederalGrowthReviewV12QuestionSequence.js');
...
function main() {
  ...
  wrapper = wrapper
    ...
    .replace('_runtime_RenderP2GCFederalGrowthReviewV10CopyFix.js', '_runtime_RenderP2GCFederalGrowthReviewV12QuestionSequence.js')
    .replace("path.join(ROOT,'DATA','reusable_demo','v10')", "path.join(ROOT,'DATA','reusable_demo','v12')")
    .replace('P2GC_Federal_Growth_Review_Demo_V10.mp4', 'P2GC_Federal_Growth_Review_Demo_V12.mp4')
    .replace('latest_p2gc_v10_render.json', 'latest_p2gc_v12_render.json')
  ...
  if (!wrapper.includes('applyV11Transforms(src)') || !wrapper.includes('applyV12Tighten(src)')) {
    throw new Error('V12_WRAPPER_TRANSFORM_INJECTION_FAILED');
  }
