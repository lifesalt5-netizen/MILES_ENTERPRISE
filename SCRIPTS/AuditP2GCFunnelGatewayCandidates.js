...
function discoverRunPy(){
  ...
  const ps=`$roots=@('${base}',$env:USERPROFILE,$env:ProgramData,$env:TEMP) | Where-Object {$_ -and (Test-Path -LiteralPath $_)} | Select-Object -Unique; $out=@(); foreach($r in $roots){$out += Get-ChildItem -LiteralPath $r -Filter run.py -File -Recurse -ErrorAction SilentlyContinue | Select-Object -First 40 -ExpandProperty FullName}; $out | Select-Object -Unique -First 80 | ConvertTo-Json -Compress`;
...
function inspect(file){
  try{
    ...
    const text=fs.readFileSync(file,'utf8');const lower=text.toLowerCase();
    const markers={
    ...
    const score=(markers.port8779?8:0)+(markers.reviewPath?3:0)+(markers.apiReview?4:0)+(markers.adminReview?6:0)+(markers.adminBlock?3:0)+(markers.localhost8792?7:0)+(markers.proxy?4:0)+(markers.server?2:0);
...
function main(){
  const paths=discoverRunPy();const candidates=paths.map(inspect).filter(Boolean).sort((a,b)=>(b.score||0)-(a.score||0));const best=candidates[0]||null;
  const highConfidence=Boolean(best&&best.score>=20&&best.markers?.port8779&&best.markers?.server);
  const result={ok:true,service:'P2GC_FUNNEL_GATEWAY_CANDIDATE_AUDIT',observedAt:new Date().toISOString(),candidateCount:candidates.length,bestCandidate:best,highConfidence,candidates:candidates.slice(0,25),safety:{readOnly:true,filesChanged:false,networkChanged:false,processChanged:false}};
