# INSTALL_MILES_BUILD_026_GOVERNED_EXECUTIVE_DISPATCH.ps1
# Complete replacement of AutonomousCOOLoopService.js only.
# Connects Build 025 Executive COO authorized work to the existing WorkQueue and WorkflowService.
...
$BackupRoot = Join-Path $Root "_BACKUPS\BUILD_026_$Stamp"
$ReportDir = Join-Path $Root "DATA\build_026"
$TestDir = Join-Path $Root "TESTS"
$Target = "SERVICES\AutonomousCOOLoopService.js"
...
if (-not (Test-Path $TargetPath)) {
    throw "Missing authoritative AutonomousCOOLoopService: $TargetPath"
}
...
Write-Host ""
Write-Host "============================================================"
Write-Host "MILES BUILD 026"
Write-Host "Governed Executive Dispatch"
Write-Host "Only AutonomousCOOLoopService.js will be replaced."
Write-Host "============================================================"
...
$TestPath = Join-Path $TestDir "Test_Build026_GovernedExecutiveDispatch.js"
...
Write-Host ""
Write-Host "=== BUILD 026 SYNTAX VALIDATION ==="
...
$Files = @(
    ...
    ".\TESTS\Test_Build026_GovernedExecutiveDispatch.js"
)
...
Write-Host ""
Write-Host "=== BUILD 026 AUTOMATED TESTS ==="

$Output = & node $TestPath 2>&1
$ExitCode = $LASTEXITCODE
$Report = Join-Path $ReportDir "build_026_test_$Stamp.txt"

$Output | Tee-Object -FilePath $Report

if ($ExitCode -ne 0) {
    throw "Build 026 tests failed. Restore from $BackupRoot"
}

$Manifest = [ordered]@{
    ok = $true
    build = "026"
    name = "Governed Executive Dispatch"
    ...
    capabilities = @(
        "Consume Executive COO authorized work",
        "Create persistent work-queue items",
        "Dispatch authorized objectives through WorkflowService",
        "Record CEO-protected decisions",
        "Block CEO-protected workflow execution"
    )
    report = $Report
}
...
$Manifest |
    ...
    Set-Content -Path (Join-Path $ReportDir "build_026_manifest_$Stamp.json") -Encoding UTF8

Write-Host ""
Write-Host "============================================================"
Write-Host "BUILD 026 GOVERNED EXECUTIVE DISPATCH INSTALLED AND VERIFIED"
Write-Host "============================================================"
